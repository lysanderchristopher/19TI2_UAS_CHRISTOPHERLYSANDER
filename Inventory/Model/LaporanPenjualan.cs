...
namespace Inventory
{
    class LaporanPenjualan
    {
        ...
        public List<LaporanPenjualan> loadHistory()
        {
            List<LaporanPenjualan> dataLaporan = new List<LaporanPenjualan>();
            try
            {
                String sql = "SELECT * FROM HistoryPenjualan";
        ...
        public void updateHistory(List<LaporanPenjualan> dataLaporan)
        {
            //delete old data on Access
            dbConn.Open();
            OleDbCommand tempCmd = new OleDbCommand("DELETE FROM HistoryPenjualan", dbConn);
            ...
            for (int i = 0; i < dataLaporan.Count(); i = i + 1)
            {
                ...
                OleDbCommand cmd = new OleDbCommand("INSERT INTO HistoryPenjualan (Item, [Purchased Date], [Quantity], [Price]) Values(@Item, @Tanggal, @Quantity, @Price)");
